...
import { Drawer, Box, Typography, Paper, TextField } from "@mui/material";
...
export default ({ open, onClose }: { open: boolean; onClose: any }) => {
  ...
  const addLink = useCallback(() => {
    ...
    let tempLink = link;
    if (
      tempLink.indexOf("https://") === -1 &&
      tempLink.indexOf("http://") === -1
    ) {
      tempLink = "https://" + link;
    }
  ...
  const saveChanges = () => {
    dispatch(
      updateDao({ url: DAO?.url, payload: { name, description, image } })
    );
  ...
  const deleteLink = (response: any, item: any) => {
    if (item.link.indexOf("github.") > -1) {
      ...
      if (ob) {
        axiosHttp
          .get(
            `utility/getGithubAccessToken?code=${response.code}&repoInfo=${repoInfo}`
          )
          .then((res) => {
            if (res.data) {
              dispatch(
        ...
        let links = daoLinks.filter(
          (l: any) => !(l.title === item.title && l.link === item.link)
        );
        setDaoLinks(links);
      }
    } else {
  ...
  const handleOnServerAdded = (serverId: any) => {
    if (importRoles) {
      axiosHttp
        .post(`discord/guild/${serverId}/sync-roles`, {
          daoId: _get(DAO, "_id"),
        })
        .then((res) => {
          addLink();
  ...
  const handleGithub = (token: any, repoInfo: any) => {
    axiosHttp
      .get(
        `utility/get-issues?token=${token}&repoInfo=${repoInfo}&daoId=${_get(
          DAO,
          "_id",
          null
        )}`
      ...
      .then((result) => {
        ...
        if (result.data.message === "error") {
          ...
          const e = document.getElementById("error-msg");
          e!.innerHTML =
            "Please check repository for ownership or typography error";
          setIsAuthenticating(false);
          return;
        } else {
    ...
    >
      ...
      >
        ...
        <Box className={classes.heading}>
          ...
          <TextInput
            placeholder="https://app.loamads.xyz/Name"
            sx={{ marginTop: "10px" }}
            fullWidth
          />
        </Box>
        ...
        <Box className={classes.heading}>
          ...
          >
            ...
            <Box className={classes.optionalHeading}>Optional</Box>
          </Box>
          <Box className={classes.dropzoneContainer}>
            ...
            >
              Accepted formats: jpg, svg or png
            </Typography>
          </Box>
        </Box>

        <Box className={classes.borderContainer}></Box>
        ...
        <Box className={classes.addLinksContainer}>
          ...
          >
            <TextInput
              ...
              sx={{ width: "144px", marginRight: "12px" }}
              value={linkTitle}
              onChange={(e: any) => setLinkTitle(e.target.value)}
            ></TextInput>
            <TextInput
              placeholder="http://discord..."
              sx={{ width: "193px", marginRight: "13px" }}
              value={link}
                  ...
                  <>
                    {
                      <AddGithubLink
                        ...
                        okButton={undefined}
                        onGuildCreateSuccess={undefined}
                        innerRef={undefined}
        ...
        {link && link.indexOf("github.") > -1 ? (
          <Box style={{ marginLeft: "93px", marginTop: "10px" }}>
            <Switch
              checked={pullIssues}
              onChange={() => setPullIssues((prev) => !prev)}
              label="IMPORT ISSUES"
            />
          </Box>
        ) : (
          <>
            {link && link.indexOf("discord.") > -1 ? (
              <Box style={{ marginLeft: "93px", marginTop: "10px" }}>
                <Switch
                  checked={importRoles}
                  onChange={() => setImportRoles((prev) => !prev)}
                  label="IMPORT ROLES"
                />
              </Box>
            ) : null}
