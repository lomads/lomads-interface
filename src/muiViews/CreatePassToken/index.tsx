...
import useMeta, { getDeployParams, getMetaDetails } from "../../hooks/useMeta";
...
const useStyles = makeStyles((theme: any) => ({
  ...
  paper: {
    ...
    boxShadow:
      "3px 5px 4px rgba(27, 43, 65, 0.05), -3px -3px 8px rgba(201, 75, 50, 0.1) !important",
    borderRadius: "5px",
  },
  paperDetails: {
    width: 479,
...
export default () => {
  ...
  useEffect(() => {
    axiosHttp.get("contract").then((res) => {
  ...
  //       ? require("abis/SBTDeployer.json")
  //       : chainId === SupportedChainId.POLYGON
  //       ? require("abisPolygon/SBTDeployer.json")
  //       : ""
  //   );
  ...
  const deployContract = async () => {
    try {
      ...
      const metaDetails = getMetaDetails(chainId!);

      const param = getDeployParams(
        params.name,
        params.symbol,
        params.mintPrice,
        params.mintToken,
      ...
      if (contractAddr) {
        const contractJSON = {
          ...
          metadata: [],
          membersList: state?.whitelisted
            ? _get(DAO, "members", []).map((m: any) => {
                return { name: m.member.name, address: m.member.wallet };
              })
            : [],
          daoId: _get(DAO, "_id", null),
        };
        axiosHttp
          .post("contract", contractJSON)
          .then((res) => {
            setTimeout(() => {
              window.location.href = `/${_get(DAO, "url", "")}`;
            }, 500);
          })
  ...
  const handleUploadWhitelist = (data: Array<any>) => {
    ...
    setState((prev: any) => {
      return { ...prev, whitelist: { members, discounts, inviteCodes } };
    });
  };
        ...
        {editMode ? (
          <Paper className={classes.paper}>
            <TextInput
              ...
              placeholder="LMDS"
              fullWidth
              label="Symbol of the Pass Token"
            />
            <Box mt={4}>
              <FormControl fullWidth>
                ...
                <Typography variant="subtitle2" className={classes.description}>
                  Suggested dimensions and format : 800x800, .svg or .png
                </Typography>
              </FormControl>
              <Box>
                <Dropzone
            ...
            <TextInput
              ...
              onChange={(e: any) => {
                setErrors({});
                setState((prev: any) => {
                  return { ...prev, treasury: e.target.value };
                });
              }}
            ...
            >
              <Switch
                onChange={(e: any) => {
                  setState((prev: any) => {
                    return {
                      ...
                      whitelist: prev.whitelisted
                        ? { members: [], discounts: [], inviteCodes: [] }
                        : {
                            members: _get(DAO, "members", []).map(
                              (m: any) => m.member.wallet
                            ),
                            discounts: _get(DAO, "members", []).map(
                              (m: any) => 0
                            ),
                            inviteCodes: _get(DAO, "members", []).map(
                              (m: any) => ""
                            ),
                          },
                    };
                ...
                label="Whitelisted"
              />
              {false && state?.whitelisted && (
                <XlsxUpload onComplete={handleUploadWhitelist} />
              )}
            </Box>
            {false &&
              state?.whitelisted &&
              state?.whitelist?.members.length > 0 && (
                <Box>
                        ...
                        {state?.whitelist?.members.map((m: any, _i: number) => {
                          return (
                            ...
                            >
                              <TableCell width={100}>
                                {beautifyHexToken(
                                  state?.whitelist?.members[_i]
                                )}
                              </TableCell>
                              <TableCell width={40}>
                                {state?.whitelist?.discounts[_i]}%
                              </TableCell>
                              <TableCell width={100}>
                                {state?.whitelist?.inviteCodes[_i]}
                              </TableCell>
                            </TableRow>
                          );
                        })}
            ...
            <Box my={3} mx={1}>
              <Switch
                onChange={(e: any) => {
                  setState((prev: any) => {
                    return { ...prev, priced: !prev.priced };
                  });
                }}
              ...
              <Box>
                ...
                <Box my={3} mx={1}>
                  <Switch
                    ...
                    label="Email"
                  />
                  <Typography variant="body2" className={classes.socialText}>
                    Get certain member details could be useful for the smooth
                    functioning of your organisation
                  </Typography>
                </Box>
