...
export default () => {
  ...
  const [discountCheckLoading, setDiscountCheckLoading] = useState<
    boolean | null
  >(null);
  ...
  const { mint, estimateGas, getStats, checkDiscount } = useMintSBT(
    contractId,
    contract?.version
  );
  const { onOpen, onResetAuth, authorization, isAuthenticating } =
    useDCAuth("identify");
  ...
  useEffect(() => {
    if (!DAO) dispatch(getDao(daoURL));
  }, [DAO]);

  const updateMetadata = async () => {
    const personalDetails = _find(
      metadata?.attributes,
      (attr: any) => attr.trait_type === "Personal Details"
    );
  ...
  useEffect(() => {
    if (DAO?.sbt) {
      setContract(DAO?.sbt);
      setPrice((prev: any) => {
        return { ...prev, mintPrice: _get(DAO?.sbt, "mintPrice", 0) };
      });
    }
  }, [DAO?.sbt]);
  ...
  useEffect(() => {
    if (
      ((prevAuth == undefined && authorization) ||
        (prevAuth && authorization && prevAuth !== authorization)) &&
      hasClickedAuth
    ) {
      handleDiscord();
    }
  ...
  const handleDiscord = async () => {
    ...
    setHasClickedAuth(true);
    if (!authorization) return onOpen();
    ...
    const calculatePriceAndGasFees = async () => {
      try {
        if (contract?.mintPrice && chainId) {
          ...
          if (!isNativeToken) coinId = "usd-coin";
            ...
            let estimateinUsd =
              parseFloat(
                ethers.utils.formatUnits(
                  estimateTransactionCost.toString(),
                  "gwei"
                )
              ) * price;
  ...
  const handleUpdateMetadata = async () => {
    ...
    if (
      contract?.contactDetail.indexOf("discord") > -1 &&
      (state?.discord == null || state?.discord == "")
    ) {
      err["discord"] = "Enter valid discord";
    }
    if (
      contract?.contactDetail.indexOf("github") > -1 &&
      (state?.github == null || state?.github == "")
    ) {
      err["github"] = "Enter valid github";
    }
    if (
      contract?.contactDetail.indexOf("telegram") > -1 &&
      (state?.telegram == null || state?.telegram == "")
    ) {
      err["telegram"] = "Enter valid telegram";
    }
    if (Object.keys(err).length > 0) return setErrors(err);
    const msg = await encryptMessage(
      JSON.stringify({
        email: _get(state, "email", ""),
        discord: _get(state, "discord", ""),
        telegram: _get(state, "telegram", ""),
        github: _get(state, "github", ""),
      })
    );
    ...
    if (
      ...
      metadataJSON = {
        ...
        attributes: [
          ...metadataJSON.attributes,
          { trait_type: "Personal Details", value: msg },
        ],
      };
    }

    axiosHttp
      .patch(`metadata/${_get(DAO, "sbt._id")}`, metadataJSON)
      .then(async (res) => {
  ...
  const handleMint = async () => {
    ...
    try {
      ...
      if (contract?.version === "1") {
        const ipfsURL: string = await axiosHttp
          .post(`metadata/ipfs-metadata`, {
            metadata: metadataJSON,
            tokenURI: `${process.env.REACT_APP_NODE_BASE_URL}/v1/${contract?.address}/${tokenId}`,
          })
          .then((res) => res.data);
            ...
            <Grid ml={1} container>
              ...
              <Grid item xs={12} sm={6}>
                ...
                >
                  ...
                  {balance === 1 && metadata && (
                    <Typography
                      style={{ fontSize: 26, marginLeft: 16, fontWeight: 700 }}
                    >
                      #{metadata?.id}
                    </Typography>
                  )}
                  ...
                  >
                    ...
                    >
                      ...
                      <Box
                        ...
                        style={{
                          flexGrow: 1,
                          borderBottom: "1px dotted rgba(27, 43, 65, 0.2)",
                        }}
                      ></Box>
                      <Box
                        style={{
                      ...
                      >
                        ...
                        >
                          $
                          {parseFloat(_get(price, "mintPriceinUsd", 0)).toFixed(
                            2
                          )}{" "}
                          /
                        </Typography>
                        ...
                        >
                          {parseFloat(_get(price, "mintPrice", 0)).toFixed(5)}{" "}
                          {contract?.mintPriceToken === USDC_GOERLI.address ||
                          contract?.mintPriceToken === USDC_POLYGON.address
                            ? "USDC"
                            : "ETH"}
                        </Typography>
                      </Box>
                      ...
                      >
                        ...
                        >
                          ...
                          <Typography
                            ...
                            style={{ fontSize: 16, fontWeight: 700 }}
                          >
                            {parseFloat(_get(price, "gas", 0)).toFixed(5)}{" "}
                            {"ETH"}
                          </Typography>
                        </Box>
                      </Box>
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            $
                            {(
                              parseFloat(_get(price, "mintPriceinUsd", 0)) +
                              parseFloat(_get(price, "estimateinUsd", 0))
                            ).toFixed(2)}{" "}
                            /
                          </Typography>
                          <Typography
                          ...
                          >
                            {(
                              parseFloat(_get(price, "mintPrice", 0)) +
                              parseFloat(_get(price, "gas", 0))
                            ).toFixed(5)}{" "}
                            {contract?.mintPriceToken === USDC_GOERLI.address ||
                            contract?.mintPriceToken === USDC_POLYGON.address
                              ? "USDC"
                      ...
                      metadata?.attributes.map((attribute: any) => {
                        if (
                          [
                            ...
                            "tasks",
                            "task_names",
                          ].indexOf(attribute.trait_type) === -1
                        ) {
                          return (
                            <Box
                              py={2}
                            ...
                            >
                              ...
                              <Box mx={2} mt={1} style={{ flexGrow: 1 }}></Box>
                              <Box
                                style={{
                                  display: "flex",
                                  flexDirection: "row",
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                {balance === 1 ? "Update details" : "Paiement"}
              </Typography>
            </Box>
            <Box px={12}>
              <TextInput
              ...
              <Box mt={4}>
                ...
                {contract?.contactDetail.indexOf("email") > -1 && (
                  ...
                  >
                    <img
                      ...
                      src={state["email"] ? EmailGreenSVG : EmailSVG}
                    />
                    <TextInput
                      value={state["email"]}
                      error={errors["email"]}
                      ...
                      <>
                        ...
                        >
                          ...
                          <Button
                            ...
                            color="secondary"
                          >
                            <Typography style={{ color: palette.primary.main }}>
                ...
                {contract?.contactDetail.indexOf("github") > -1 && (
                  ...
                  >
                    <img
                      style={{ width: 36, marginRight: 12 }}
                      src={state["github"] ? GithubGreenSVG : GithubSVG}
                    />
                    <TextInput
                      value={state["github"]}
                      error={errors["github"]}
                      ...
                      onChange={(e: any) =>
                        setState((prev: any) => {
                          return { ...prev, github: e.target.value };
                        })
                      }
                ...
                {contract?.contactDetail.indexOf("telegram") > -1 && (
                  ...
                  >
                    <img
                      style={{ width: 36, marginRight: 12 }}
                      src={state["telegram"] ? TelegramGreenSVG : TelegramSVG}
                    />
                    <TextInput
              ...
              {networkError && typeof networkError === "string" && (
                <Typography
                  my={2}
                  textAlign="center"
                  color="error"
