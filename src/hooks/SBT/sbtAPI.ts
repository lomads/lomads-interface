...
export const APInewSBTtoken = async (metadata: metadataAPI) => {
    // const config = {
    //     method: "post",
    //     url: process.env.REACT_APP_NODE_BASE_URL + `/metadata/add-metadata/${metadata.contract}`,
    //     headers: {
    //         "Content-Type": "application/json",
    //     },
    ...
    try {
        const req = await axiosHttp.post(`metadata/${metadata.contract}`, metadata);
...
export const APIgetContract = async (contract: string) => {
    ...
    // }
    try {
        const req = await axiosHttp.get(`contract/${contract}`);
        return req;
    }
    catch (e) {
        return false;
