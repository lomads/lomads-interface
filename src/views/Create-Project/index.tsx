...
import { Container, Grid, Typography, Box, FormLabel, Dialog } from "@mui/material"
...
import AddMemberPopup from "views/Dashboard/AddMemberPopup";
				...
				<Box>
					...
					{nextButtonClicked &&
						<Box className={classes.memberDetails}>
							<Box className={classes.memberHeader}>
								...
								<Button variant='contained' color='secondary' size="small" onClick={addNewMemberHandler}>Add New Member</Button>
							</Box>
							{memberList.map(item => {
					...
					{nextButtonClicked &&
						<Box className={classes.bottomButtonBox}>
							<Button size="small" color="primary" style={{ marginRight: '1.5rem', background: '#FFFFFF' }} onClick={addMoreDetailHandler}>Add More Detail</Button>
							<Button size="small" variant="contained">Create Workspace</Button>
						</Box>
					}
					<Dialog open={addMember} onClose={closeAddMemberDialog}>
						<AddMemberPopup closePopup={closeAddMemberDialog} />
					</Dialog>
				</Box>
			}
			{addMoreDetail && <AddMoreDetail />}
		</Box>
	)
}