...
const AllProjectTasks = () => {
    ...
    useEffect(() => {
        if (DAO && user) {
            ...
            const myTasks = _get(DAO, 'tasks', []).filter(task => task.creator !== user._id && task.project?._id === projectId && (!task.deletedAt && !task.archivedAt && !task.draftedAt && ((task.contributionType === 'open' && !task.isSingleContributor) || !isOthersApproved(task)) && (_find(task.members, m => m.member.wallet.toLowerCase() === account.toLowerCase()) || amIEligible(task) || (task.contributionType === 'open' && !task.isSingleContributor))))
            setMyTasks(_orderBy(myTasks, i => moment(i.deadline).unix(), 'desc'))
            let manageTasks = _get(DAO, 'tasks', []).filter(task => task.project?._id === projectId && !task.deletedAt && !task.archivedAt && !task.draftedAt && (task.creator === user._id || task.reviewer === user._id));
