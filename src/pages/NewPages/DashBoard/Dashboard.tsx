...
import Notifications from "muiComponents/Notifications";
...
const Dashboard = () => {
  ...
  const { user, DAO, DAOList, DAOLoading } = useAppSelector(
  ...
  const [showCreateRecurring, setShowCreateRecurring] =
  ...
  const { myRole, displayRole, permissions, can, isSafeOwner } = useRole(
    DAO,
    account
  );
  ...
  const handleRenderRole = () => {
    const user = _find(
      _get(DAO, "members", []),
      (m) =>
        _get(m, "member.wallet", "").toLowerCase() === account?.toLowerCase()
    );
    return _get(user, "role", "").replaceAll("_", " ").toLowerCase();
  };

  useEffect(() => {
    if (account) getENSName(account);
  }, [account]);
  ...
  useEffect(() => {
    if (chainId && account) {
      if (!DAOList) dispatch(loadDao({ chainId }));
      else {
        if (DAOList && DAOList.length == 0) {
          if (!daoURL) navigate(`/createorg`);
          else {
            if (!DAO || (DAO && DAO.url !== daoURL)) dispatch(getDao(daoURL));
          }
        } else if (DAOList && DAOList.length > 0) {
          if (!daoURL) navigate(`/${DAOList[0].url}`);
            ...
            if (DAO?.sbt?.whitelisted) {
              if (
                _find(
                  DAO.members,
                  (member) =>
                    member.member.wallet.toLowerCase() === account.toLowerCase()
                )
              ) {
  ...
  const ownersCount = async (_safeAddress: string) => {
    ...
    const dao = await axiosHttp.patch(
  ...
  useEffect(() => {
    if (DAO && chainId && DAO.chainId === chainId) {
      const prevShow = localStorage.getItem(
        `lmds_notification_count_${DAO._id}_show`
      );
      if (!prevShow || (prevShow && prevShow === "1"))
        return setShowNotification(true);
      if (prevShow === "0") {
        if (pendingTransactions?.count) {
          const prevCount = localStorage.getItem(
            `lmds_notification_count_${DAO._id}`
          );
          ...
          console.log(
            "prevCount",
            prevCount &&
              prevCount.toString() === pendingTransactions?.count.toString()
          );
          if (
            prevCount &&
            prevCount.toString() === pendingTransactions?.count.toString()
  ...
  const swtBalance = useMemo(() => {
    if (DAO && user) {
      const swt = _find(
        _get(user, "earnings", []),
        (e: any) => e.currency === "SWEAT" && e.daoId === _get(DAO, "_id")
      );
      if (swt) return _get(swt, "value", 0);
    }
    return 0;
  }, [user, DAO]);
  ...
  const tokenDollarBalance = useMemo(() => {
    if (DAO && user) {
      let usdVal = 0;
      const myTokens = _get(user, "earnings", []).filter(
        (e: any) => e.daoId === _get(DAO, "_id")
      );
        ...
        const safeTkn = _find(
          ...
          (st: any) =>
            (st.tokenAddress
              ...
              : process.env.REACT_APP_GOERLI_TOKEN_ADDRESS) === myToken.currency
        );
        if (safeTkn) {
          console.log("safeTkn", safeTkn, myToken);
          usdVal =
            usdVal +
            +_get(safeTkn, "fiatConversion", 0) * _get(myToken, "value", 0);
        }
      }
  ...
  const endWalkThrough = () => {
    dispatch(
      updateUserOnboardingCount({ payload: { daoId: _get(DAO, "_id", "") } })
    );
  ...
  const incrementWalkThroughSteps = () => {
    ...
    if (nextObj.step >= 6) {
      if (nextObj.step === 6) {
        anchorRef.current.style.boxShadow =
          "0px 0px 20px rgba(181, 28, 72, 0.6)";
      } else {
  ...
  return (
    <>
      ...
      {(showWalkThrough || isHelpIconOpen) && <div className="overlay"></div>}
      <WalkThroughModal
        ...
        <div className="DAOdetails">
          ...
          >
            <div>
              ...
              >
                <div className="DAOname">{_get(DAO, "name", "")}</div>
        ...
        {(can(myRole, "transaction.view") || isSafeOwner) &&
          ...
          daoURL === _get(DAO, "url", "") && (
            <TreasuryCard
              ...
              account={account}
              onChangePendingTransactions={(tx: any) =>
      ...
      <div
        className={`help-option ${isHelpIconOpen ? "z-index-60" : ""}`}
        id="question-mark"
      ...
      >
        ...
        <img
          src={
            (showWalkThrough && currWalkThroughObj.step === 7) || isHelpIconOpen
              ? questionMarkDark
              : questionMarkLight
          }
        />
      ...
      {showCreateRecurring && (
        <CreateRecurring
          ...
          onRecurringPaymentCreated={() => treasuryRef?.current?.reload()}
          toggleShowCreateRecurring={toggleShowCreateRecurring}
        />
      )}
    </>
